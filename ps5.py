...
def filter(pixels, color):
    ...
    returns: list of pixels in same format as earlier funictions,
    transformed by matrix multiplication
    """
    #makes the matrix with color
    color_change = make_matrix(color)
    #new list of pixels to be returned
    new_pic_list = []
    #goes through each pixel tuple 
    for i in pixels:        
        #for each tuple in the pixel - multiplies it by the color matrix
        num_tuple = matrix_multiply(color_change, i) #matrix
        #new list to save the rounded tuple numbers
        tuple_list = []
        for x in num_tuple:
            tuple_list.append(round(x))
        #changes the list into tuple
        tuple_list = tuple(tuple_list)
        #adds the tuple to the new list for the new picture
        new_pic_list.append(tuple_list)
    return new_pic_list


def extract_end_bits(num_bits, pixel):
    """
    Extracts the last num_bits bits of each value of a gven pixel. 
    ...
    """
    #Need to count 2 to the power of each num bit, 
    #Must be exclusive start value to inclusive end value
    num = num_bits-1
    bits = ""
    #IF BW 
    if type(pixel) == int:
        for i in range(num, -1, -1):
            #if the int is even then 0 else 1
            if int(pixel / (2**(i))) %2 == 0:
                bits += "0"
            else:
                bits += "1"
        bits = int(bits, 2)
    ...
    #IF COLOR
        ...
        bit_list = []
        for i in pixel:
            #goes through each number in the tuple
            ...
            bits = ""
        #Change the binary into decimal list
        for i in range(len(bit_list)):
            bit_list[i] = int(bit_list[i], 2)
        #Change list into a tuple
        bit_list = tuple(bit_list)
        return bit_list
...
def reveal_bw_image(filename):
    ...
    #Go through each number in the pixel, 1 or 0
    for i in range(len(pixels)):
        #finds end bits then rescales the image 
        # for black and white its either black or white so 0 or 255
        pixels[i] = extract_end_bits(1, pixels[i])*255
    #Change the pixels back to image, making them B/W
    im = pix_to_img(pixels, (width, height), 'L')
    return im
...
def reveal_color_image(filename):
    ...
    pixels = img_to_pix(filename)
    pix_list = []
    #each pixel is a tuple
    for i in pixels:
        #Changes each number in the tuple to the last 3 bits
        #Rescales by multiplying by 255/7
        R = int(extract_end_bits(3, i[0])* 255/7)
        G = int(extract_end_bits(3, i[1])* 255/7)
        B = int(extract_end_bits(3, i[2])* 255/7)
        pix_list.append((R, G, B))
    #Change pixels back to image, making them color
    im = pix_to_img(pix_list, (width, height), 'RGB')
    return im
...
def draw_kerb(filename, kerb):
    ...
    draw.text((0, 0), kerb, "black", font=font)
    idx = filename.find(".")
